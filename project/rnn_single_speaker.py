...
class PadSequence:
    def __call__(self, batch):
        ...
        sequences = [torch.tensor(
            x[0].reshape(-1, 13), dtype=torch.float32) for x in sorted_batch]
...
class LSTMNetwork(nn.Module):
    def __init__(
        ...
        if not full_dropout:
            self.linear_layers = nn.ModuleList([nn.Linear(
                hidden_dim_linear[i], hidden_dim_linear[i + 1]) for i in range(len(hidden_dim_linear) - 1)])
        else:
            linear_layers = []
            for i in range(len(hidden_dim_linear) - 1):
                linear_layers.append(
                    nn.Linear(hidden_dim_linear[i], hidden_dim_linear[i + 1]))
    ...
    def init_weights(self):
        # Initialize the weights of the forget gate to a higher value to
    ...
    def forward(self, x, seq_lengths):
        packed = nn.utils.rnn.pack_padded_sequence(
            ...
            seq_lengths,
            batch_first=True,
            enforce_sorted=True).to(
            self.device)
        output, _ = self.rnn(packed)
        output, _ = nn.utils.rnn.pad_packed_sequence(output, batch_first=True)
        B, N, F = output.shape
        idx = torch.tensor([l - 1 for l in seq_lengths],
                           dtype=torch.int64).to(self.device)

        output = torch.gather(
            output, dim=1, index=idx.view(
                B, 1, 1).expand(
                B, N, F))[
            :, -1, :]
...
if __name__ == "__main__":
    ...
    for speaker in speakers:
        train, test = load_and_split(meta_filename="SDR_metadata.tsv", speaker=speaker)
        ...
        model_name = f"single_speaker_rnn_{speaker}_hs{hidden_size}_bs{batch_size}_nl{n_rnn_layers}_dr{dropout}_lr{learning_rate}"
        if full_dropout:
            model_name += '_fdr'
    ...
    # dev_data = AudioDataset(dev, num_mels=num_mels)
        ...
        train_loader = DataLoader(
            train_data,
            batch_size=batch_size,
            shuffle=not single_batch_overfit,
            collate_fn=PadSequence(),
            num_workers=16)
    # dev_loader = DataLoader(dev_data, batch_size=batch_size, shuffle=True, collate_fn=PadSequence(), num_workers=16)
        ...
        for epoch in range(num_epochs):
            ...
            # lengths = lengths.to(device)
                ...
                train_correct += (torch.argmax(output, dim=1)
                              == label).sum().item()
            ...
            if not single_batch_overfit:
                with torch.no_grad():
                    for features, lengths, label in test_loader:
                        ...
                        val_correct += (torch.argmax(output, dim=1)
                                    == label).sum().item()
        ...
        # Plot loss and acc
            ...
            print(
                'Epoch: {}, Train Loss: {:.4f}, Train Accuracy: {:.4f}, Val Loss: {:.4f}, Val Accuracy: {:.4f}'.format(
                    epoch +
                    1,
                    train_loss,
                    train_accuracy,
